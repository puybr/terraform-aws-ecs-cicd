...
#### _Option 1_ - Configure a new `test` AWS CLI profile and add the keys:
...
```sh
[profile test]
sso_session = terraform-sso
sso_account_id = 444455556666
sso_role_name = readOnly
region = eu-west-2
output = json

[sso-session terraform-sso]
...
- Add the backend configuration to the `terraform` providers block:
```tf
# terraform {
    backend "s3" {
